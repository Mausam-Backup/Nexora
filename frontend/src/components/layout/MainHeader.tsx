...
export const MainHeader: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full px-3 sm:px-6 pt-3 pb-2 select-none">
      {/* Floating Capsule Header Container (Warm Terracotta-Espresso matching pink theme) */}
      <div className="mx-auto max-w-6xl w-full rounded-full bg-[#241411] text-white border border-[#44251F] shadow-2xl px-4 sm:px-6 py-2 sm:py-2.5 flex items-center justify-between gap-4 transition-all">
        ...
        <Link 
          ...
          title="Nexora ERP"
        >
          <div className="h-7 w-7 sm:h-8 sm:w-8 rounded-full bg-white flex items-center justify-center text-[#241411] shadow-sm group-hover:scale-105 transition-transform shrink-0">
            {/* Geometric 4-circle clover emblem */}
            ...
            return (
              <NavLink
                ...
                className={`transition-colors hover:text-white py-1 relative whitespace-nowrap ${
                  isActive 
                    ? 'text-white font-bold after:absolute after:-bottom-1.5 after:left-1/2 after:-translate-x-1/2 after:w-1.5 after:h-1.5 after:rounded-full after:bg-[#E07A5F]' 
                    : 'text-[#D8B4AD] hover:text-white'
                }`}
              >
                {link.label}
              </NavLink>
        ...
        {/* Right: Capsule Action Button & Quick Tools */}
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <button
                className="relative h-8 w-8 rounded-full bg-[#341B16] hover:bg-[#43231D] text-[#E8C5BE] hover:text-white border border-[#4E2A23] flex items-center justify-center transition-colors cursor-pointer"
                title="Institutional Notifications"
              >
                <Bell className="h-3.5 w-3.5" />
                <span className="absolute top-1.5 right-1.5 h-1.5 w-1.5 rounded-full bg-emerald-400 ring-2 ring-[#241411] animate-pulse" />
              </button>
            </PopoverTrigger>
            <PopoverContent align="end" className="w-80 p-0 rounded-2xl shadow-2xl border-[#44251F] bg-[#241411] text-white">
              <div className="p-3 border-b border-[#44251F] flex items-center justify-between">
                <span className="text-xs font-bold font-serif">Notifications</span>
                <Badge variant="outline" className="text-[10px] border-[#5A2E24] text-[#E8C5BE]">3 Unread</Badge>
              </div>
              <div className="divide-y divide-[#44251F]/80 max-h-72 overflow-y-auto text-xs">
                {notifications.map((n) => (
                  <div key={n.id} className="p-3 hover:bg-[#341B16]/60 transition-colors space-y-0.5">
                    <p className="font-bold text-white leading-tight">{n.title}</p>
                    <p className="text-[11px] text-[#D8B4AD] leading-snug">{n.desc}</p>
                    <p className="text-[10px] text-[#A88880] pt-0.5">{n.time}</p>
                  </div>
                ))}
              </div>
            </PopoverContent>
          ...
          <DemoRoleSwitcher
            ...
            customTrigger={
              <button
                type="button"
                className="rounded-full bg-[#381D17] hover:bg-[#48251E] text-white border border-[#5A2E24] pl-3.5 sm:pl-4 pr-1.5 py-1 flex items-center gap-2 sm:gap-2.5 text-xs font-semibold transition-all shadow-md group hover:border-[#7A3F33] cursor-pointer shrink-0"
              >
                <span className="whitespace-nowrap font-medium tracking-tight">
                  {roleCapsuleLabel}
                </span>
                <span className="h-6 w-6 rounded-full bg-[#8F361E] text-white flex items-center justify-center font-bold text-xs shrink-0 group-hover:scale-105 transition-transform shadow-xs">
          ...
          <button
            ...
            className="lg:hidden h-8 w-8 rounded-full bg-[#341B16] hover:bg-[#43231D] text-[#E8C5BE] hover:text-white flex items-center justify-center border border-[#4E2A23] transition-colors"
          >
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden mt-2 mx-auto max-w-6xl w-full rounded-2xl bg-[#241411] border border-[#44251F] shadow-2xl p-4 space-y-2 text-xs font-semibold animate-in fade-in-50 duration-200">
          <div className="text-[10px] text-[#D8B4AD] uppercase tracking-wider px-2 font-serif">Quick Navigation</div>
            ...
            {navLinks.map((link) => (
              <Link
                ...
                className={`p-2 rounded-xl transition-colors ${
                  location.pathname === link.to ? 'bg-white/15 text-white font-bold' : 'text-[#D8B4AD] hover:text-white hover:bg-[#341B16]'
                }`}
              >
                {link.label}
              </Link>
